...
import os
#from mlflow_works.mlflow_tests.tracking_client.mlflow_trackingclient import MlflowClient
...
'''
mlfc = MlflowClient()
#exps = mlfc.list_experiments(view_type=None)
run = mlfc.search_runs(experiment_ids="0")
#pp = pprint.PrettyPrinter(indent=4)
#pp.pprint (run[0].to_dictionary())
run_dict = run[0].to_dictionary()
for f in mlfc.list_artifacts(run_id=run_dict['info']['run_id']):
    print("file: "+f.path)

#mlfc.download_artifacts(run_id=run_dict['info']['run_id'],path="model",dst_path="/Users/rijupahwa/Downloads")
...
for metric in metrics:
    ...
    perf_data={}
    perf_data.update (
        {"mlflow_experiment_id": "0" , "mlflow_metric": metric , "mlflow_run_id": run_dict[ 'info' ][ 'run_id' ]})
    metric_data = []
    for met in  mlfc.get_metric_history(run_id=run_dict['info']['run_id'],key=metric):
        print(run_dict['info']['run_id'],met.key, met.value, met.timestamp, met.step)
        metric_data.append({"value":met.value,"timestamp":met.timestamp,"step":met.step})

    perf_data.update({"mlflow_performance_data":metric_data})
'''